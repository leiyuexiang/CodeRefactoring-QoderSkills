...
**确定性保证**：本技能采用三级确定性分类链（Level 1 精确映射 → Level 2 关键词匹配 → Level 3 默认 custom），所有分类由显式映射表和模式匹配驱动，零依赖业务语义解读。给定相同输入工程，任意次数执行产出完全一致。

**执行链路定位**：本技能在工程级执行链路中位于 **Step6**（Step1→Step2→...→**Step6（当前）**→...→Step9）。其中 Step3~Step7 为代码级重构步骤，内部编号为 S1~S5，本技能对应 **S4**（S1→S2→S3→**S4（当前）**→S5）。S4 前置依赖 S3（Service层治理/Step5），后续为 S5（命名规范/Step7）。
...
- S4 检查项清单（5 项：S4-01 至 S4-05）
- 三级确定性分类链（分类 → 路径转换 → 迁移）
- 集中重构策略（禁止就地重构）
- 检查流程与修复流程
- 安全约束（C-01 至 C-06、S-01 至 S-18、D-01 至 D-06）
- 一致性约束（D-01 至 D-06，确保多次执行结果一致）
...
- Controller 层使用 `@Controller` 或 `@RestController` 注解（`@FeignClient` 排除在外）
- 项目根目录（或所在 module）存在 `pom.xml`（用于版本检测）
- S3（Service层治理）已完成
- 修复前用户需确认修复计划
- 分类完全由确定性映射表驱动，不依赖人工业务判断

## 目录结构

```
Step6-controller-governance-versioned-Controller层治理/
