...
8. **【新增】强制完整性原则**：所有 5 大修复步骤必须完整执行，关键步骤（Step 3: Model 层分类）不可跳过，确保多次执行结果一致
...
**检查项**：
1. 检查 `model/` 根目录散落文件数（应为 0）
2. 检查 `model/dto/`、`model/vo/`、`model/po/` 目录是否存在
3. 检查 `dao/mapper/`、`dao/entity/` 目录是否存在
4. 检查是否存在 `imp/` 非标准命名目录
...
**若治理完成度 = 100%**：提示用户"该模块已完成 Step4 治理，无需重复执行"
...
```
修复计划确认
├── [A] 全部执行（推荐）- 执行所有 5 大步骤
├── [B] 仅执行检查，不修复
└── [C] 分步执行（高级）
    ├── ☑ Step 1: 目录命名修正
    ├── ☑ Step 2: DAO 层归位
    ├── ☑ Step 3: Model 层分类 ← 【强制必选，不可取消勾选】
    ├── ☑ Step 4: 创建缺失目录
    └── ☑ Step 5: 全局验证
```

**约束**：
- 选择 [C] 时，Step 3（Model 层分类）必须勾选，不可取消
- 若用户坚持跳过 Step 3，必须输出警告："跳过此步骤将导致 Model 层文件未归档，多次执行结果不一致"
- **推荐**：引导用户选择 [A] 全部执行，确保治理完整性
...
| 校验点 | 检查内容 | 预期结果 | 失败处理 |
|--------|---------|---------|---------|
| V1 | `model/` 根目录散落文件数 | = 0 | 输出未归档文件列表，提示重新执行 Step 3 |
| V2 | 所有 `*DTO.java` 文件位置 | 在 `model/dto/` 下 | 列出异常文件，提示分类错误 |
| V3 | 所有 `*VO.java`/`*BO.java` 文件位置 | 在 `model/vo/` 下 | 列出异常文件，提示分类错误 |
| V4 | 所有 `*PO.java`/`*Po.java` 文件位置 | 在 `model/po/` 下 | 列出异常文件，提示分类错误 |
| V5 | 无后缀文件归档 | 在 `model/po/` 下 | 列出未归档文件，提示执行兜底规则 |
| V6 | `dao/` 根目录非接口文件 | 应在 `impl/` 或 `mapper/` 下 | 列出异常文件，提示 DAO 归位 |
...
═══════════════════════════════════════
模块: {module-name}
├─ [PASS] V1: model/ 根目录散落文件 = 0
├─ [PASS] V2: DTO 文件归档正确
├─ [PASS] V3: VO/BO 文件归档正确
├─ [PASS] V4: PO 文件归档正确
├─ [PASS] V5: 无后缀文件已归入 po/
└─ [PASS] V6: DAO 层归位正确
───────────────────────────────────────
...
═══════════════════════════════════════
模块: {module-name}
├─ [FAIL] V1: model/ 根目录散落文件 = 65
│   └─ 未归档文件: AccountManageDTO.java, AgencyUser.java, ...
├─ [PASS] V2: DTO 文件归档正确
└─ ...
───────────────────────────────────────
治理状态: 未完成
建议操作: 重新执行 Step 3（Model 层分类）
═══════════════════════════════════════
```
