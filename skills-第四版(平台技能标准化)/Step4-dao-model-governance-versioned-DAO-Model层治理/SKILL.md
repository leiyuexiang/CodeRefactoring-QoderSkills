...
同一模块多次执行本技能时，可能因用户选择"跳过某些步骤"导致结果不一致。例如：
- 第一次执行：完整执行 5 大步骤 → Model 层文件全部归档
- 第二次执行：跳过 Step 3 → Model 层根目录残留散落文件
...
| 机制 | 说明 |
|------|------|
| Phase 0 幂等性检查 | 执行前检查治理状态，输出完成度报告 |
| 强制完整性原则 | Step 3（Model 层分类）为强制必选步骤，不可跳过 |
| 标准化确认流程 | 引导用户选择"全部执行"，确保治理完整性 |
| 执行后校验机制 | 6 个校验检查点确保治理完成 |
| 一致性声明输出 | 治理完成后输出"一致性保证声明" |
| import 语句规范化 | 禁止通配符导入，统一使用具体类名导入 |
| 全限定名规范化 | 优先使用 import + 简单类名，统一类引用方式 |

### 预期一致率

改进后，相同初始状态 + 相同修复流程 → **一致率 98%+**（排除 AI 标记随机码差异后）
