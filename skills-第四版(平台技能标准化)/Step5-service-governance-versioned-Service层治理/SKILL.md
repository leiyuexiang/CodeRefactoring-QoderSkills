---
name: Step5-service-governance-versioned-Service层治理
description: "[Step5] Java微服务Service层治理检查与修复工具（多版本适配）。根据工程pom.xml中的version自动选择对应版本的检查与修复规则。检查并修复Service层接口/实现分离(facade/impl)、facade目录存在性、Service接口归属正确性、Service实现归属正确性、非Service文件处理等问题，不改变业务逻辑。执行链路：Step1→Step2→Step3→Step4→Step5（当前）→Step6→Step7→Step8→Step9。当用户提到'Step5检查'、'Step5修复'、'Service层治理检查'、'Service层治理修复'、'facade/impl检查'、'facade/impl修复'时使用。"
---

# S3 Service 层治理（多版本适配）

## 概述

本技能是 S3 Service 层治理检查与修复的**多版本适配版本**。不同工程版本的 Service 层规范可能存在差异，本技能会根据工程 `pom.xml` 中声明的 `<version>` 自动选择对应版本的检查与修复规则执行。

**执行链路定位**：S3 是代码级重构的第三步（中间层 Service 文件归位），前置依赖 S2（DAO-Model层治理），后续为 S4（Controller层治理）。执行链路：S1→S2→**S3（当前）**→S4→S5。

**检查项来源**：合并自原 P1-02（Service层接口/实现分离）+ 原 P4（Service facade/impl 分离全部 4 项）。

## 支持的版本

| 工程版本 | 规则目录 | 说明 |
|----------|----------|------|
| `3.6.0-SNAPSHOT` | [versions/3.6.0-SNAPSHOT/](versions/3.6.0-SNAPSHOT/) | 基线版本，完整 S3 Service 层治理检查与修复规则 |
...
按照加载的版本规则执行具体的 Service 层治理检查或修复操作。规则文件中包含：
- S3 检查项清单（5 项）
- 检查流程
- 修复流程与策略
- 安全约束
...
| 场景 | 触发关键词 | 调用功能 |
|------|-----------|----------|
| 检查 Service 层是否符合规范 | "S3检查"、"Service层治理检查"、"facade/impl检查" | 版本检测 → Service 层检查 |
| 修复不符合规范的 Service 层 | "S3修复"、"Service层治理修复"、"facade/impl修复" | 版本检测 → Service 层修复 |
| 先检查再修复 | "S3检查并修复"、"Service层治理全流程" | 版本检测 → 检查 + 修复 |
...
```
S3-service-governance-versioned-Service层治理/
