...
必须扫描以下所有目录位置，确保不遗漏任何Service文件：

| 扫描路径模式 | 说明 |
|-------------|------|
| `{module}/src/main/java/**/service/*.java` | Service根目录 |
| `{module}/src/main/java/**/config/*/service/*.java` | 业务子目录下的service |
| `{module}/src/main/java/**/view/*/service/*.java` | 视图层下的service |
| `{module}/src/main/java/**/service/*/impl/*.java` | Service实现子目录 |

**扫描流程**：
1. 使用 Glob 工具按上述模式逐一扫描
2. 将扫描结果合并去重
3. 对每个文件按分类规则判断是否为Service接口/实现
4. 生成待迁移文件清单（包含原路径、目标路径、文件类型）

**禁止事项**：
- 禁止跳过任何业务子目录
- 禁止仅扫描service根目录而忽略业务子目录
- 禁止在未完成全部扫描前开始迁移
...
1. **残留检查**：使用 Grep 搜索原位置是否仍有符合Service接口定义的文件
   - 搜索模式：`interface.*Service\s*\{`
   - 搜索范围：排除 `service/facade/` 和 `service/impl/` 目录
...
| 场景 | 标记位置 | 示例 |
|------|---------|------|
| 新建文件 | 包围整个文件内容（第1行到最后一行） | `// @AI-Begin XXX\n全部内容\n// @AI-End XXX` |
| 修改文件 | 只包围修改的代码块 | `// @AI-Begin XXX\n修改的代码\n// @AI-End XXX` |
| 迁移文件 | 只包围修改的package声明行 | 见下方示例 |

**迁移文件AI标记示例**：
```java
// @AI-Begin A1b2C 20260401 @@Qoder
package grp.pt.frame.service.facade;
// @AI-End A1b2C 20260401 @@Qoder

import grp.frame.model.po.UserPo;
