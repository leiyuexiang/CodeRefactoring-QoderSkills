...
## 文件编码保护规范（在所有迁移操作之前执行）

### Step 0: 检测源文件编码
1. 读取源文件时，必须使用 UTF-8 编码
2. 如果读取到的中文字符出现乱码/截断（如 "?" 替代中文字符），
   立即停止该文件迁移，标记为"编码异常"
3. 写入目标文件时，必须使用 UTF-8 编码

### 文件内容完整性约束
- Read 原文件后，Write 新文件时，必须保证文件内容字节数
  与原文件差异仅限于 package 声明行的变更
- 禁止在 Write 时改变文件的行尾符（保持原文件的 LF 或 CRLF）
- 禁止在 Write 时改变文件中的空行数量
- 禁止在 Write 时改变文件中非 package/import 行的任何字符
