...
### 接口方法提取范围的强制校验步骤（不可跳过）

修复每个 S1-03 项时，**必须按以下完整步骤逐一执行**，确保每次执行提取的方法集完全一致：

```
步骤 1：全量搜索所有引用该 Impl 的 Controller
   Grep pattern: {被注入Impl类名}
   Grep path: {controller-module-src-path}
   Grep flags: -l
   → 记录结果为 ControllerList

步骤 2：对每个 Controller 逐一统计实际调用的方法名
   对 ControllerList 中的每个 Controller 文件：
   a. 确认该 Impl 在 Controller 中的字段名（如 userTempService）
   b. Grep pattern: {fieldName}\.([a-zA-Z]+)\(
      Grep path: {当前Controller文件路径}
      Grep output_mode: content
   c. 提取所有匹配的方法名，记录到 CalledMethodSet

步骤 3：汇总所有 Controller 调用方法的并集
   FinalMethodSet = 所有 Controller 的 CalledMethodSet 的并集

步骤 4：读取 Impl 源码获取精确方法签名
   对 FinalMethodSet 中的每个方法名：
   a. 在 Impl 源码中搜索该方法的完整签名（返回类型、方法名、参数列表、throws）
   b. 注意处理重载方法：同名不同参的方法，需逐一比对 Controller 中的调用参数数量和类型
   c. 将精确签名记录到 InterfaceMethodList

步骤 5：校验接口方法数量
   a. InterfaceMethodList.size() 必须 ≤ FinalMethodSet.size()（含重载）
   b. 如果接口方法数 > 并集方法数，说明提取了 Controller 未调用的方法，必须删除多余方法
   c. 将 InterfaceMethodList 作为接口的最终方法集
```

**禁止行为（与 S-10 安全约束一致）**：
- 禁止因为"方法看起来有用"就将 Controller 未调用的方法加入接口
- 禁止直接复制 Impl 的全部 public 方法到接口中
- 禁止在 Impl 有 50 个方法、Controller 只调用 10 个时，在接口中声明超过 10 个方法（不含重载额外计数）
...
### 标记最小粒度规则（强制，确保多次执行标记一致）

为确保多次执行时 AI 标记的添加位置和范围一致，**必须**按以下规则确定标记的最小粒度：

| 修改场景 | 标记最小单元 | 说明 |
|---------|------------|------|
| **单个 import 行修改**（添加/删除/替换） | 与相邻连续修改的 import 合并为一个标记块 | 如果上下相邻的 import 也被修改，全部合并到一个标记块中 |
| **单个独立 import 行修改**（上下均为未修改的 import） | 该 import 行单独作为一个标记块 | 中间有未修改行隔断时，必须独立标记 |
| **@Autowired 字段声明修改** | 注释行 + @Autowired 行 + 字段声明行 为一个最小标记单元 | 即 `// 替代原 Xxx 注入` + `@Autowired` + `private IXxxService xxx;` 三行合并标记 |
| **方法调用中字段名替换** | 仅包含被替换的那一行 | 如 `oldField.method()` → `newField.method()`，仅标记该行 |

**禁止行为**：
- 禁止将未修改的代码行包含在 AI 标记范围内（已在规则2中声明，此处再次强调）
- 禁止对同一处修改在不同执行中使用不同的标记范围（如一次标记单行 import，另一次标记整个 import 块）
- 禁止省略标记：所有修改的代码必须被 AI 标记覆盖，不得有"裸"修改
