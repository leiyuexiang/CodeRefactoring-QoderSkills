# S1 架构依赖检查与修复 - 3.6.1-SNAPSHOT 版本规则

## 版本变更说明

基于 `3.6.0-SNAPSHOT` 基线版本的增量修订版本。本版本的所有检查规则、修复规范、安全约束与 3.6.0-SNAPSHOT **完全一致**，无差异。

---

## ★★★ 强制基线文件加载指令（不可跳过）★★★

**3.6.1-SNAPSHOT 版本继承 3.6.0-SNAPSHOT 基线版本的全部规则。在执行任何检查或修复操作之前，必须先读取以下基线版本文件，获取完整的规则定义。本版本自身的简略文件仅作为索引，不包含完整规则内容。**
...
- Maven 编译验证直接用 Bash `mvn compile`，不使用 PowerShell 的 `Out-File`/`Select-String`

---

## 概述

本文件为 **3.6.1-SNAPSHOT** 版本的 S1 架构依赖检查与修复规则。
